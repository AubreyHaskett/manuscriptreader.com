// TTS Web Worker — V4 TEST BUILD
// Same as tts-worker.js, except esm.sh is asked to rebuild kokoro-js with
// @huggingface/transformers v4.x as a transitive dep. v4 ships a "Native
// WebGPU EP" that closed transformers.js#1320 (the Kokoro WebGPU corruption
// bug). This test build is here to find out whether v4 fixes the garbled
// audio on AMD iGPUs and Android without breaking kokoro-js's API surface.
// If kokoro-js's API is incompatible with v4, you'll see a load-time error.

import { KokoroTTS } from "https://esm.sh/kokoro-js@1.2.1?deps=@huggingface/transformers@4.0.0";
